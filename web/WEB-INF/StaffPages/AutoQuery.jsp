...
<form action="AutoDeleteAllServlet" method="post">
    <table class="layui-table" lay-even="" lay-skin="nob">
        <tr><td>选择</td><td>车牌号</td><td>汽车类型</td><td>品牌</td><td>汽车名称</td><td>颜色</td><td>座位数</td><td>档位类型</td><td>排量</td><td>日租金</td><td>押金</td><td>状态</td><td>操作</td></tr>
        <c:forEach items="${alist}" var="ainfo">
            <tr><td><input type="checkbox" name="select" value="${ainfo.autocard}"></td><td>${ainfo.autocard}</td><td>${ainfo.atype}</td><td>${ainfo.bname}</td><td>${ainfo.tname}</td><td>${ainfo.color}</td><td>${ainfo.seat}</td>
                <td>${ainfo.gear}</td><td>${ainfo.tubo}</td><td>${ainfo.dayrent}</td><td>${ainfo.extra}</td><td>${ainfo.state}</td>
                <td><a class="layui-btn layui-btn-mini" lay-event="detail" href="<%=basePath%>AutoDetailServlet?autocard=${ainfo.autocard}">查看</a></td></tr>
        </c:forEach>
        <tr><td colspan="13">
