...
<div class="my-btn-box">
     <span class="fl">
        <a class="layui-btn btn-add btn-default" id="btn-refresh" href="<%=basePath%>BackListServlet"><i class="layui-icon">&#x1002;</i></a>
    </span>
...
<form action="AutoDeleteAllServlet" method="post">
    <table class="layui-table" lay-even="" lay-skin="nob">
        <tr><td>选择</td><td>租赁单号</td><td>客户编号</td><td>车牌号</td><td>租赁日期</td><td>租赁天数</td><td>还车日期</td><td>实际价格</td><td>操作</td></tr>
        <c:forEach items="${backListInfoList}" var="bl">
            <tr><td><input type="checkbox" name="select" value="${bl.rno}"></td><td>${bl.rno}</td><td>${bl.cuname}</td><td>${bl.autocard}</td>
                ...
                <td><a class="layui-btn layui-btn-mini" lay-event="detail" href="<%=basePath%>BackListDetailServlet?rno=${bl.rno}&uname=${bl.cuname}&autocard=${bl.autocard}">查看</a>
                    <a class="layui-btn layui-btn-mini layui-btn-danger" lay-event="del" href="<%=basePath%>BackListDeleteServlet?rno=${bl.rno}" onclick="return del()">删除</a></td></tr>
        </c:forEach>
        <tr><td colspan="9">
