...
<div class="my-btn-box">
     <span class="fl">
        <a class="layui-btn btn-add btn-default" id="btn-refresh" href="<%=basePath%>RentAllListServlet"><i class="layui-icon">&#x1002;</i></a>
    </span>
    <form action="<%=basePath%>RentAllListServlet" method="post">
    <span class="f3">
...
<form action="AutoDeleteAllServlet" method="post">
    <table class="layui-table" lay-even="" lay-skin="nob">
        <tr><td>选择</td><td>租赁单号</td><td>客户编号</td><td>车牌号</td><td>租赁日期</td><td>租赁天数</td><td>租赁总金额</td><td>操作人</td><td>操作</td></tr>
        <c:forEach items="${rentListInfoList}" var="rl">
            <tr><td><input type="checkbox" name="select" value="${rl.rno}"></td><td>${rl.rno}</td><td>${rl.cuname}</td><td>${rl.autocard}</td>
                <td> <fmt:formatDate value="${rl.rdate}" pattern="yyyy/MM/dd"/> </td><td>${rl.rdays}</td><td>${rl.allmoney}</td><td>${uname}</td>
                <td><a class="layui-btn layui-btn-mini" lay-event="detail" href="<%=basePath%>RentAllListDetailServlet?rno=${rl.rno}&uname=${rl.cuname}&autocard=${rl.autocard}">查看</a>
                    <a class="layui-btn layui-btn-mini layui-btn-normal" lay-event="edit" href="<%=basePath%>BackListAddServlet?rno=${rl.rno}&uname=${rl.cuname}&autocard=${rl.autocard}">还车</a>
