    ...
    <table class="layui-table" lay-even="" lay-skin="nob">
        ...
        <c:forEach items="${changeMoneyList}" var="cminfo">
            <tr><td><input type="checkbox" name="select" value="${cminfo.cid}"></td><td>${cminfo.cid}</td><td>${cminfo.autocard}</td><td>${cminfo.cdate}</td><td>${cminfo.brent}</td><td>${cminfo.arent}</td>
                <td><a class="layui-btn layui-btn-mini" lay-event="detail" href="<%=basePath%>ChangeMoneyDetailServlet?cid=${cminfo.cid}">查看</a>
                    <a class="layui-btn layui-btn-mini layui-btn-danger" lay-event="del" href="<%=basePath%>ChangeMoneyDeleteServlet?cid=${cminfo.cid}" onclick="return del()">删除</a></td></tr>
        </c:forEach>
        <tr><td colspan="7">
