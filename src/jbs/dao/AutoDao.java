...
public class AutoDao extends BaseDao{
    ...
    public int AutoUpdate(Auto auto) throws Exception {
        int iRet;
        this.openConnection();
        String sql = "update auto set autocard=?,bno=?,tno=?,color=?,seat=?,gear=?,tubo=?,dayrent=?,tname=?,extra=? where autocard=?";
        PreparedStatement ps = this.conn.prepareStatement(sql);
        ps.setString(1,auto.getAutocard());
        ps.setString(2,auto.getBno());
        ps.setString(3,auto.getTno());
        ...
        ps.setString(7,auto.getTubo());
        ps.setDouble(8,auto.getDayrent());
        ps.setString(9,auto.getTname());
        ps.setDouble(10,auto.getExtra());
        ps.setString(11,auto.getAutocard());
    ...
    public List<AutoInfo> getStaffAutoInfo(String atype, String bname, String tname, String tubo,TurnPage tp) throws Exception{
        List<AutoInfo> alist;
        String sql = "select a.autocard,c.atype,b.bname,a.tname,a.color,a.seat,a.gear,a.tubo,a.dayrent,a.extra,state "
                + "from auto a,brand b,autotype c,qstate d "
                + "where a.bno = b.bno and a.tno = c.tno and a.autocard=d.autocard";
        if(atype!=null&&!atype.equals("")){
            sql = sql+"and c.atype ="+atype;
        }
        if(bname!=null&&!bname.equals("")){
